...
public final class AdvancedSettingsActivity extends LocalizedActivity implements RunCoordinator.Listener {
    private static final int REQUEST_IMPORT_ZIP = 1001;
    private static final int REQUEST_EXPORT_JSON = 1002;
    static final String EXTRA_WORKLOAD_POSITION = "workload_position";
    ...
    private void buildUi() {
        ...
        int requestedWorkload = getIntent().hasExtra(EXTRA_WORKLOAD_POSITION)
                ...
                : preferences.getInt("workload_position", 0);
        workloadSpinner.setSelection(Math.max(0, Math.min(WORKLOAD_IDS.length - 1, requestedWorkload)));
        ...
        workloadNote.setTextColor(AmaralColors.TEXT_SECONDARY);
        ...
        resultPreview.setBackgroundColor(AmaralColors.SURFACE);
        ...
        GitHubDeviceFlow.authorize(BuildConfig.GITHUB_CLIENT_ID, new GitHubDeviceFlow.Callback() {
            @Override public void onCode(String userCode, String verificationUri) {
                runOnUiThread(() -> {
                    ...
                    new LocalizedAlertDialogBuilder(AdvancedSettingsActivity.this)
