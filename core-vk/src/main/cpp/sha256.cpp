...
std::string sha256Hex(const uint8_t* data, size_t length) {
    ...
    for (size_t i = 0; i < fullBlocks; ++i) {
        state.processBlock(data + i * 64);
    }

    uint8_t tail[128];
    const size_t remaining = length - fullBlocks * 64;
    std::memcpy(tail, data + fullBlocks * 64, remaining);
    tail[remaining] = 0x80;

    // The length field needs nine bytes of room (one for the 0x80 marker plus
    // eight for the count), so a nearly full block spills into a second one.
    const size_t padded = (remaining + 9 <= 64) ? 64 : 128;
    std::memset(tail + remaining + 1, 0, padded - remaining - 1);

    const uint64_t bitLength = static_cast<uint64_t>(length) * 8;
    for (int i = 0; i < 8; ++i) {
        tail[padded - 1 - i] = static_cast<uint8_t>((bitLength >> (i * 8)) & 0xFF);
    }

    state.processBlock(tail);
    if (padded == 128) state.processBlock(tail + 64);
